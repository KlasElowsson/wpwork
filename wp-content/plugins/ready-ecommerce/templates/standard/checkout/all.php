...
jQuery(document).ready(function(){
	...
	jQuery('#toeGuestCheckout').change(function(){
		if(jQuery('#toeGuestCheckout').attr('checked')) {
			jQuery('#toe_checkout_form_1 [name^=registration]').not('[name=shipping_module]').setReadonly();
			jQuery('#guestCheckoutIndicator').show();
			jQuery(".toe_checkout_part_registrationForm table").hide('slow');
			jQuery(".toe_checkout_part_registrationForm table input[type='text']").each(function() { jQuery(this).val("GuestCheckout"); });
		} else  {
		
			jQuery(".toe_checkout_part_registrationForm table input[type='text']").each(function() { jQuery(this).val(""); });
			jQuery('#toe_checkout_form_1 [name^=registration]').not('[name=shipping_module]').unsetReadonly();
			jQuery('#guestCheckoutIndicator').hide();
			jQuery(".toe_checkout_part_registrationForm table").show('slow');
		}
	});
	
	jQuery('#toeGuestCheckout').trigger('click');
